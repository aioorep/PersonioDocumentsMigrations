...
def debug_employee():
    ...
    # Fetch raw employee with includes
    r1 = requests.get(f"{BASE_URL}/company/employees/{emp_id}", headers=headers, params={"includes": "documents"}, timeout=30)
    r2 = requests.get(f"{BASE_URL}/company/employees/{emp_id}/documents", headers=headers, timeout=30)

    return jsonify({
        "employee_id": emp_id,
        "email": target_emails[0],
        "employee_with_includes": r1.json() if r1.ok else {"status": r1.status_code, "body": r1.text},
        "documents_endpoint": r2.json() if r2.ok else {"status": r2.status_code, "body": r2.text},
    })
