...
@app.route("/api/debug-employee", methods=["GET"])
def debug_employee():
    """Return raw Personio API response for the first matched employee — helps diagnose document structure."""
    src_id     = session.get("src_client_id", "")
    src_secret = session.get("src_client_secret", "")
    if not src_id or not src_secret:
        return jsonify({"error": "No credentials in session. Run pre-flight first."}), 400
    try:
        token = authenticate(src_id, src_secret)
    ...
    if not target_emails:
        return jsonify({"error": "No emails in session."}), 400

    employees = get_employees(token)
    emp = next((e for e in employees if extract_info(e)["email"] == target_emails[0]), None)
    if not emp:
        return jsonify({"error": f"Employee {target_emails[0]} not found in source."}), 404

    emp_id = extract_info(emp)["id"]
    headers = {"Authorization": f"Bearer {token}"}

    # Try every possible document-related endpoint
    endpoints = {
        "documents":    f"{BASE_URL}/company/employees/{emp_id}/documents",
        "attachments":  f"{BASE_URL}/company/employees/{emp_id}/attachments",
        "files":        f"{BASE_URL}/company/employees/{emp_id}/files",
        "all_documents":f"{BASE_URL}/company/documents",
    }
    results = {}
    for key, url in endpoints.items():
        r = requests.get(url, headers=headers, timeout=30)
        results[key] = {"status": r.status_code, "body": r.json() if r.ok else r.text[:500]}

    # Also try employee with different includes params
    for inc in ["documents", "attachments", "files", "document"]:
        r = requests.get(f"{BASE_URL}/company/employees/{emp_id}", headers=headers, params={"includes": inc}, timeout=30)
        if r.ok:
            data = r.json().get("data", {}).get("attributes", {})
            # Only return keys that look document-related
            doc_keys = {k: v for k, v in data.items() if any(word in k.lower() for word in ["doc", "file", "attach"])}
            results[f"includes_{inc}"] = doc_keys if doc_keys else f"No document-related keys found (total keys: {len(data)})"
